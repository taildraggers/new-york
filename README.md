...
Daily aggregator of New York-located taildragger classified listings,
...
repos' approach, with the state filter swapped to "NY".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, NY` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in New York"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/new-york/`
...
```html
<iframe
  src="https://taildraggers.github.io/new-york/"
  title="Taildragger Ads in New York"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_NEW_YORK_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
