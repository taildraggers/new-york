"""Aggregates New York-located listings from every taildraggers.com
...
ones whose location ends in ", NY".
...
Jersey/New Mexico repos' approach, with the state filter swapped to "NY".
"""
...
PAGE_TITLE = "Taildragger Ads in New York"
STATE_ABBR = "NY"
...
_NEW_YORK_LOCATION_RE = re.compile(r",\s*NY\s*$", re.IGNORECASE)


def _is_new_york(location: str) -> bool:
    return bool(_NEW_YORK_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        new_york_rows = [row for row in rows if _is_new_york(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(new_york_rows)} in New York")
        all_listings.extend(new_york_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No New York listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique New York listings: {len(listings)}")
